...
// Manually trigger a profile refresh (you can also hook this to a cron).
// ---------------------------------------------------------------------
export async function GET() {
  try {
    ...
    // 1) Fetch existing long-term profile, if any
    const existingSummary = await getUserProfileSummary(userId);

    // 2) Fetch a chunk of recent memories (larger than chat's slice)
...
- Keep it concise but rich (about 400–800 words max).
